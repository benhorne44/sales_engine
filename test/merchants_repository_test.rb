...
class MerchantRepositoryTest < MiniTest::Test
  ...
  def test_it_finds_all_by_name
    response = merchant_repository.find_all_by_name('Schroeder-Jerde')
  ...
  def test_it_finds_one_by_name
    response = merchant_repository.find_by_name('Schroeder-Jerde')
    assert_equal 'Schroeder-Jerde', response.name
  end

  def test_it_finds_by_id
    response = merchant_repository.find_by_id(3)
    assert_equal 3, response.id
  end

  def test_it_finds_by_created_at
    response = merchant_repository.find_by_created_at('2012-03-27 14:53:59 UTC')
    assert_equal '2012-03-27 14:53:59 UTC', response.created_at
  end

  def test_it_finds_all_by_created_at
    response = merchant_repository.find_all_by_created_at('2012-03-27 14:53:59 UTC')
    assert_equal 6, response.count
    response.each { |merchant| assert_equal '2012-03-27 14:53:59 UTC', merchant.created_at }
  end

  def test_it_finds_by_updated_at
    response = merchant_repository.find_by_updated_at('2012-03-27 14:53:59 UTC')
    assert_equal '2012-03-27 14:53:59 UTC', response.updated_at
  end

  def test_it_finds_all_by_updated_at
    response = merchant_repository.find_all_by_updated_at('2012-03-27 14:53:59 UTC')
    assert_equal 6, response.count
